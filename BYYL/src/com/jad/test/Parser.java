...
/**
 * 自动去直接左递归、自动生成FIRST、FOLLOW集合、自动构建预测分析表、输入单词串给出推导过程
 */
public class Parser {
    ...
    public static boolean isLL1() {
            ...
            for (int i = 0; i < FORM.length; i++) {
                for (int j = 0; j < FORM[0].length; j++) {
                    if (FORM[i][j] == null) {
                        ...
                        System.out.format("%-10s\t", FORM[i][j]);
                        if (i > 0 && j > 0) {
        ...
        public static void printAutoPre (String str){
            ...
            while (!stack.isEmpty()) {
                ...
                if (left.equals(right) && right.equals("#")) {
                    isSuccess = true;
                    // System.out.format("%-3d\t#%-8s\t#%-8s\t%-10s\n", step++, "", "", "分析成功");
                    System.out.format("%-3d\t#%-8s\t#%-8s\t%-10s\n", step++, "", "", "分析成功");
                    break;
                }
                ...
                if (left.equals(right)) {
                    ...
                    // System.out.format("%-3d\t%-8s\t%-8s\t%-10s\n", step++, stackStr, queueStr, "匹配成功" + left);
                    System.out.format("%-3d\t%-8s\t%-8s\t%-10s\n", step++, stackStr, queueStr, "匹配成功" + left);
                    stack.pop();
                    queue.poll();
                    continue;
                ...
                if (preMap.containsKey(left + right)) {
                    String stackStr = String.join("", stack.toArray(new String[stack.size()]));
                    String queueStr = String.join("", queue.toArray(new String[queue.size()]));
                    // System.out.format("%-3d\t%-8s\t%-8s\t%-10s\n", step++, stackStr, queueStr, "用" + left + "→" + preMap.get(left + right) + "," + right + "逆序进栈");
                    System.out.format("%-3d\t%-8s\t%-8s\t%-10s\n", step++, stackStr, queueStr, "用" + left + "→" + preMap.get(left + right) + "," + right);
                    stack.pop();
        ...
        public static void identifyVnVt (ArrayList < String > list) {
            ...
            // 打印Vn、Vt
            System.out.println("\n非终结符Vn集合:\t{" + String.join("、", VN.toArray(new String[VN.size()])) + "}");
            System.out.println("终结符Vt集合:\t{" + String.join("、", VT.toArray(new String[VT.size()])) + "}");
        ...
        public static void findFirst () {
            ...
            while (it.hasNext()) {
                ...
                // 输出key的FIRST集合
                // System.out.println("\tFIRST(" + key + ")={" + String.join("、", firstCell.toArray(new String[firstCell.size()])) + "}");
                System.out.println("\tFIRST={" + String.join("、", firstCell.toArray(new String[firstCell.size()])) + "}");
            }
        }
        ...
        public static void findFollow () {
            ...
            while (itF.hasNext()) {
                ...
                System.out.println("\tFOLLOW={" + String.join("、", f.toArray(new String[f.size()])) + "}");
            }
        }
        ...
        public static ArrayList<String> InitGrammar () {
            ...
            // 方式2
            // result.add("E→T|E+T|E-T");
            // result.add("T→F|T*F|T/F");
            // result.add("F→(E)|i'");
            return result;
        }
    }
