...
public class ImportExcel {
	...
	public boolean checkHeader(List<String> nameList) {
		Row headRow = getRow(headerNum);
		int maxCellNum = getLastCellNum();
		if (maxCellNum < nameList.size()) {
			return false;
		}
		for (int i = 0; i < nameList.size(); i++) {
			Object headName = getCellValue(headRow, i);
			if (nameList.get(i).equals(headName)) {
				continue;
			} else {
				return false;
			}
	...
	public String getSheetNames() {
		...
		for (int i = 0; i < sheetNum; i++) {
			String name = this.wb.getSheetName(i);
			if (i == 0) {
				sb.append(name);
			} else {
				sb.insert(0, name + ";");
