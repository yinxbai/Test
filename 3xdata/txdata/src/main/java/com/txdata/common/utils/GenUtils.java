...
import com.txdata.common.domain.ColumnDO;
import com.txdata.common.domain.TableDO;
...
public class GenUtils {
	...
	private static String templateFolder = "/templates/common/generator";//ftl模板位置
	
	static {
		configuration = new freemarker.template.Configuration(new Version("2.3.0"));
		configuration.setDefaultEncoding("utf-8");
		configuration.setClassForTemplateLoading(GenUtils.class, templateFolder);
	}

	public static List<String> getTemplates() {
		List<String> templates = new ArrayList<String>();
		templates.add("domain.java.ftl");
...
//		templates.add("templates/common/generator/menu.sql.vm");
	...
	public static void generatorCode(Map<String, String> table, List<Map<String, String>> columns,
			ZipOutputStream zip) throws TemplateNotFoundException, MalformedTemplateNameException, ParseException, IOException, TemplateException {
		// 配置信息
		...
		Boolean hasBigDecimal = false;//默认不存在大数字字段。
		Boolean hasDate = false;//判断
		// 列信息
		...
		for (Map<String, String> column : columns) {
			...
			if(attrType.equals("BigDecimal")) {//判断是否属于大字段
				hasBigDecimal = true;
			}
			if(attrType.equals("Date")) {//判断是否属于时间类型的字段
				hasDate = true;
			}
			// 是否主键
...
//		VelocityContext context = new VelocityContext(map);
		...
		for (String template : templates) {
			freemarker.template.Template template2 = configuration.getTemplate(template);
			StringWriter sw = new StringWriter();
			template2.process(map, sw);
			// 渲染模板
//			StringWriter sw = new StringWriter();
...
//		}
	...
	public static String getFileName(String template, String classname, String className, String packageName) {
		...
		if (template.contains("Mapper.xml.ftl")) {
			return "main" + File.separator + "resources" + File.separator + "mybatis/modules/daily" + File.separator + packageName
